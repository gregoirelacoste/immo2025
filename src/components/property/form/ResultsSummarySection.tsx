...
export default function ResultsSummarySection({ calcs, showAirbnb = true, isBeginner }: Props) {
  const verdict = isBeginner ? getVerdict(calcs) : null;
      ...
      <div className={`grid grid-cols-1 ${showAirbnb ? "md:grid-cols-2" : ""} gap-4 md:gap-6`}>
        <div className="bg-white/10 rounded-lg p-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span>{isBeginner ? "Rendement net" : "Rentabilité nette"}</span>
              <span className="font-bold">{formatPercent(calcs.net_yield)}</span>
            </div>
            <div className="flex justify-between border-t border-white/20 pt-2">
              <span>{isBeginner ? "Reste dans votre poche / mois" : "Cash-flow mensuel"}</span>
