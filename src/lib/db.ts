...
/** Cast a libsql Row to a typed object */
...
export async function getPropertyBySourceUrl(
  ...
  const db = await getDb();
  const result = await db.execute({
    sql: "SELECT * FROM properties WHERE source_url = ? LIMIT 1",
    args: [sourceUrl],
  });
  return result.rows[0] ? rowAs<Property>(result.rows[0]) : undefined;
}
  ...
  await db.execute({
    sql: `
      INSERT INTO properties (
        id, address, city, postal_code, purchase_price, surface, property_type, description,
